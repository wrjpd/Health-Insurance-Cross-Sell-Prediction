...
class CategoricalVsCategoricalAnalysis(BivariateAnalysisStrategy):
    def analyze(self, df: pd.DataFrame, feature1: str, feature2: str):
        """
        Plots the relationship between two categorical features using a Contingency Table.

        Parameters:
        df (pd.DataFrame): The dataframe containing the data.
        feature1 (str): The name of the categorical feature/column to be analyzed.
        feature2 (str): The name of the categorical feature/column to be analyzed.

        Returns:
        None: Displays a Contingency Table showing the relationship between the categoricals features.
        """
        # Countplot
        print('\nCount')
        ...
        print('\nContigency Table')
        normalize = 'index'
        table = pd.crosstab(index = df[feature1], columns = df[feature2], normalize = normalize, margins = True)
